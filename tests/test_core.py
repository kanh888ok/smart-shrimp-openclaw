...
from src.desensitizer import desensitize_text
...
def test_desensitizer_masks_common_sensitive_fields():
    text = (
        "联系人张三，电话13812345678，邮箱test@example.com，"
        "统一社会信用代码91330100MA2ABCDEF1，身份证110101199001011234。"
    )
    sanitized = desensitize_text(text)
    assert "13812345678" not in sanitized
    assert "test@example.com" not in sanitized
    assert "91330100MA2ABCDEF1" not in sanitized
    assert "110101199001011234" not in sanitized
    assert "[手机号已脱敏]" in sanitized
