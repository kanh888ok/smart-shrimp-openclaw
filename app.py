...
def optional_feature_available(feature_name, modules, install_cmd="pip install -r requirements-advanced.txt"):
    """高级可选功能依赖检查：缺依赖时禁用，不抛红色错误"""
    ...
    st.info(
        f"{feature_name} 属于高级可选功能。当前环境未安装：{', '.join(missing)}。"
        "基础数据分析和基础模型功能可正常使用。"
    )

    with st.expander("需要启用该高级功能时安装"):
...
def create_metric_card(title, value, suffix="", delta=None):
    """创建指标卡片"""
    delta_str = f"{delta}" if delta is not None else ""

    if delta is not None:
        color = "green" if isinstance(delta, (int, float)) and delta > 0 else "red"
        delta_html = f'<div style="font-size: 0.8rem; color: {color};">{delta_str}</div>'
    else:
        delta_html = ""
...
def data_analysis_page(df, df_enhanced):
    ...
    with tab5:
        ...
        if '预警等级' in df_enhanced.columns:
            ...
            alert_df = df_enhanced.copy()
            alert_df['预警等级_数值'] = alert_df['预警等级'].map(alert_order)
            ...
            for level, color in [
                ...
                data = alert_df[alert_df['预警等级'] == level]
                if len(data) > 0:
                    fig.add_trace(go.Scatter(
                        x=data['日期'],
                        y=data['预警等级_数值'],
...
def model_evaluation_page(df_enhanced, predictor):
    ...
    with tab1:
        ...
        if hasattr(predictor, "y_test") and hasattr(predictor, "y_pred"):
            ...
            st.info("当前模型对象没有保存 y_test / y_pred，无法绘制预测-实际散点图。")
    ...
    with tab3:
        ...
        st.info("💡 调整下方参数后，系统会基于当前训练好的模型重新预测产量。")
        ...
        with col2:
            dissolved_oxygen = st.slider("溶解氧 (mg/L)", 3.0, 10.0, 6.5, 0.1)
        ...
        if st.button("🔮 预测产量", use_container_width=True):
            if predictor.model is None:
                st.error("模型尚未训练，无法预测。")
            else:
                feature_names = getattr(predictor, "feature_names", None)
                if feature_names is None:
                    feature_names = list(predictor.X.columns)

                baseline_df = df_enhanced[feature_names].dropna()

                if baseline_df.empty:
                    st.error("没有可用的完整特征数据，无法预测。")
                else:
                    # 使用最后一条完整样本作为基准，再用滑块覆盖关键变量
                    features = baseline_df.iloc[-1].to_dict()

                    if "水温 (°C)" in features:
                        features["水温 (°C)"] = temp
                    if "溶解氧 (mg/L)" in features:
                        features["溶解氧 (mg/L)"] = dissolved_oxygen
                    if "pH 值" in features:
                        features["pH 值"] = ph
                    ...
                    if dissolved_oxygen < 5.0:
                        stress_score += 3
                    if temp < 24.0 or temp > 32.0:
                        stress_score += 2
                    if ph < 7.5 or ph > 8.5:
                        stress_score += 2

                    if "环境压力指数" in features:
                        features["环境压力指数"] = stress_score

                    X_new = pd.DataFrame([features], columns=feature_names)
                    pred_yield = float(predictor.model.predict(X_new)[0])

                    st.success(f"预计产量：{pred_yield:.0f} kg")

                    st.caption(
                        "说明：该预测以当前数据集中最后一条完整样本作为基准，"
                        "仅覆盖水温、溶解氧、pH 和环境压力指数。"
                    )

                    with st.expander("查看本次预测输入特征"):
                        st.json(features)
...
def advanced_models_page(df, df_enhanced):
    ...
    with tab1:
        ...
        st.caption("当前公开演示版本使用模块默认训练参数；滑块参数仅作为界面展示。")

        deep_learning_ready = optional_feature_available(
            "深度学习模型",
            ["torch"]
        )
        ...
        if st.button(
            f"🚀 训练 {model_type} 模型",
            use_container_width=True,
            disabled=not deep_learning_ready
        ):
            try:
                ...
                st.warning(f"缺少可选依赖：{e.name}。请安装高级依赖后再使用。")
    ...
    with tab2:
        ...
        time_series_ready = optional_feature_available(
            "时序预测模型",
            ["prophet", "pmdarima"]
        )
        ...
        if st.button(
            f"🔮 运行 {ts_model} 预测",
            use_container_width=True,
            disabled=not time_series_ready
        ):
            try:
    ...
    with tab3:
        ...
        **融合策略**：
        ...
        ensemble_ready = optional_feature_available(
            "模型融合",
            ["xgboost", "lightgbm"]
        )
    ...
    with tab4:
        ...
        **优化参数**：
        ...
        optuna_ready = optional_feature_available(
            "超参数优化",
            ["optuna"]
        )

        st.caption("当前公开演示版本使用模块默认优化次数；滑块参数仅作为界面展示。")

        if st.button(
            "🔍 开始优化",
            use_container_width=True,
            disabled=not optuna_ready
        ):
            try:
    ...
    with tab5:
        ...
        **SHAP (SHapley Additive exPlanations)**：
        ...
        shap_ready = optional_feature_available(
            "模型解释",
            ["shap"]
        )
    ...
    with tab6:
        ...
        comparison_ready = optional_feature_available(
            "全模型对比",
            ["xgboost", "lightgbm"]
        )
    ...
    with tab7:
        ...
        **融合策略**：
        ...
        if fusion_strategy == "hybrid":
            fusion_ready = optional_feature_available(
                "混合多模态融合",
                ["torch"]
            )
